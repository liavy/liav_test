...
/**
 * Title: ListWebServicesCommand
 * Description: Class that is representing list_ws shell command from the webservices group  
...
public class ListWebServicesCommand implements Command {
  private String nl = SystemProperties.getProperty("line.separator");
    ...
    try {
      ...
      serverLocation.catching("Unexpected exception occurred in calling list_ws command.", e);
      try {
        environment.getErrorStream().write(("Unexpected exception occurred in calling list_ws command: " + e.getMessage()).getBytes()); //$JL-I18N$
  ...
  private String getAllServices(boolean withDetails) {
    String resultString = "";
    ConfigurationContext configurationContext = WebServicesContainer.getServiceContext().getConfigurationContext();
    ...
    if (appConfigContexts == null || appConfigContexts.size() == 0) {
      resultString = "No webservices have been registered/started."  + nl + nl;
    }
  ...
  private String getServices(String applicationName, boolean withDetails) {
    ApplicationConfigurationContext applicationContext = (ApplicationConfigurationContext)WebServicesContainer.getServiceContext().getConfigurationContext().getApplicationConfigurationContexts().get(applicationName);
    if (applicationContext == null) {
      if (applicationName.startsWith("-")) {
       return getHelpMessage();
      } else {
        return "Webservice with Application name " + applicationName + " does not exists." + nl + nl;
      }
    }
  ...
  private String getServices(String applicationName, String serviceName, boolean withDetails) {
    ...
    String helpMessage = "For every application prints Application name,  Service name, Service contextRoot, BindingData name, BindingData url" + nl + nl
                         + "USAGE    : " + getName() + " [<application name>] [<service name>] -[<parameter>] "  + nl
                         + "Parameter: " + nl
                         + " d : lists additional information for the specified application: " + nl
                         + " Service name, Service contextRoot, BindingData name, BindingData url, BindingData interface-id, BindingData interface-mapping-id." + nl + nl;
