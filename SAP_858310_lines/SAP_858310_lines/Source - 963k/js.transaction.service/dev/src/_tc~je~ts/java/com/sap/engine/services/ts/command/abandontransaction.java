...
public class AbandonTransaction extends AbstractTxCommand {

	private static final Location LOCATION = Location.getLocation(AbandonTransaction.class);
	...
	public void exec(Environment env, InputStream is, OutputStream os, String[] params) {
		...
		for(int i = 0; i < pendingTransactions.length; i++) {
			...
			try {
				...
				logException(out, "The exception(InstanceNotFoundException) occured while trying to" +
						" invoke abandon on mbean.", LOCATION, e);
				return;
			} catch (MalformedObjectNameException e) {
				logException(out, "The exception(MalformedObjectNameException) occured while trying to" +
						" invoke abandon on mbean.", LOCATION, e);
			...
			if(pendindTransactionData.getAbandonSupported()) {
				set.add(pendindTransactionData);
			}
		}

		if(set.size() == 0) {
			out.println(" There are no pending transactions for abandon.");
			return;
		}
		int num = 1;
...
//				printIfCodeIsNotOK(out, actionStatus, LOCATION);
		...
		SAP_ITSAMExtendedPendingTransactionData chosenForAbandon = it.next();
		
		CompositeData cdata = null;
		try {
			cdata = (CompositeData) mbeanServer.invoke(
					getTransactionServiceManagementMBeanObjectName(),
					"abandonTransaction", new Object[] { chosenForAbandon.getSystemId(),
							chosenForAbandon.getNodeId(), chosenForAbandon.getTmStartupTime(),
							chosenForAbandon.getTransactionSeqNumber() },
					new String[] { String.class.getName(), int.class.getName(),
							long.class.getName(), long.class.getName() });
		} catch (InstanceNotFoundException e) {
			logException(out, "The exception(InstanceNotFoundException) occured while trying to" +
...
//			}
			...
			String stackTrace = stat.getStackTrace();
	...
	public String getHelpMessage() {
		return "\n" +
		"Lists all pending transactions which can be abandoned\n" +
		"and gives the option to choose which one to be abandoned.\n\n" +
		"Usage: abandonTx  ... and then the number of chosen transaction to abandon.\n";
	}
