...
public class DbDb6Column extends DbColumn
{
	...
	public DbDb6Column(DbFactory factory, String name, int position, int javaSqlType, String dbType, long length,
			int decimals, boolean isNotNull, String defaultValue)
	{
	...
	public String getDdlDefaultValueClause() throws Exception
	{
		...
		if (defaultVal != null)
		{
			if (this.getJavaSqlType() == java.sql.Types.DATE || this.getJavaSqlType() == java.sql.Types.TIME
					|| this.getJavaSqlType() == java.sql.Types.TIMESTAMP)
			{
				if (this.getJavaSqlType() == java.sql.Types.DATE && defaultVal.equals("CURRENT DATE"))
				{
					clause = "WITH DEFAULT CURRENT DATE";
				} else if (this.getJavaSqlType() == java.sql.Types.TIME && defaultVal.equals("CURRENT TIME"))
				{
					clause = "WITH DEFAULT CURRENT TIME";
				} else if (this.getJavaSqlType() == java.sql.Types.TIMESTAMP && defaultVal.equals("CURRENT TIMESTAMP"))
				{
	...
	public boolean checkTypeAttributes()
	{
		...
		switch (this.getJavaSqlType())
		{
			...
			case java.sql.Types.NUMERIC :
				...
				{
					cat
							.errorT(loc, "checkTypeAttributes: length of decimal column {0} must range from 1 to 31 .",
									arguments);
					check = false;
				}
				...
				{
					cat.errorT(loc,
							"checkTypeAttributes: number of decimals for column {0} is higher than length of decimal column.",
							arguments);
					check = false;
				}
				break;
			...
			case java.sql.Types.CLOB :
				len = this.getLength();
				if (this.getJavaSqlType() == java.sql.Types.CLOB)
					len *= 3; // byte length
				if (len > 1073741824)
				{
	...
	protected DbColumnDifference comparePartTo(DbColumn target)
	{
		...
		// DB2 Version info may not be available if DB6Environment has not been
		// initialized
		...
		// DECIMAL(n,m) -> DECIMAL(p,q) p >= n q>=m (p-q) >= (n-m); no reorg if n
		// even and p = n+1
		//
		...
		if (originalType != targettype)
		{
			...
			cat.infoT(loc, "compareTo: column {0}: target column of type {1} differs from target type {2} .", typeargs);
				...
				switch (originalType)
				{
					...
					case java.sql.Types.SMALLINT :
						if (targettype != java.sql.Types.INTEGER
								...
								&& (targettype != java.sql.Types.DECIMAL || (targetCol.getLength() - targetCol.getDecimals() <= 4)))
						{
							convertTable = true;
						}
						break;
					...
					case java.sql.Types.INTEGER :
						if (targettype != java.sql.Types.BIGINT
								&& targettype != java.sql.Types.DOUBLE
								&& (targettype != java.sql.Types.DECIMAL || (targetCol.getLength() - targetCol.getDecimals() <= 9)))
						{
							convertTable = true;
						}
						break;
					...
					case java.sql.Types.BIGINT :
						if (targettype != java.sql.Types.DOUBLE
								&& (targettype != java.sql.Types.DECIMAL || (targetCol.getLength() - targetCol.getDecimals() <= 19)))
						{
							convertTable = true;
						}
						break;
		...
		else
		{
			...
			if (tInfo.hasLengthAttribute())
			{
				...
				if (colLength != targetLength)
				{
					...
					if (colLength > targetLength)
					{
						cat.infoT(loc, "compareTo: column {0}: table conversion is required to shrink column length.",
								arguments);
						convertTable = true;
					}
					...
					switch (originalType)
					{
						...
						case java.sql.Types.DECIMAL :
							if (assumeV8
									...
									|| ((this.getLength() - this.getDecimals()) > (targetCol.getLength() - targetCol
											.getDecimals())))
							{
								convertTable = true;
							}
			...
			// CHANGE OF DECIMALS
			//
			if (tInfo.hasDecimals() && getDecimals() != targetCol.getDecimals())
			{
				...
				// no reorg required if n even and p = n+1
				//
				if (assumeV8 || originalType != java.sql.Types.DECIMAL || (this.getDecimals() > targetCol.getDecimals())
						|| ((this.getLength() - this.getDecimals()) > (targetCol.getLength() - targetCol.getDecimals())))
				{
					convertTable = true;
				}
			}
			...
			} else if (!targetCol.isNotNull())
			{
				...
				if (primKey != null)
				{
					...
					while (!colInPrimKey && iter.hasNext())
					{
						...
						{
							cat.infoT(loc,
									"compareTo: column {0}: DROP NOT NULL not allowed since column occurs in primary key.",
									arguments);
							colInPrimKey = true;
							convertTable = true;
						}
		...
		if ((origDefault == null && targetDefault != null) || (origDefault != null && targetDefault == null)
	...
	public boolean acceptedAdd()
	{
		...
		} else
		{
			...
			Object[] arguments = {this.getName()};
			cat.infoT(loc,
					"acceptedAdd: column {0} can not be added without conversion because it is NOT NULL without DEFAULT",
					arguments);
