		   ...
		   || multiIterator.hasNextWithDrop()              )
	  {
		...
		while ( multiIterator.hasNextWithTypeLenDecChange() )
		{
		  ...
		  if ( diff.getAction() == Action.ALTER )
		  {
			...
			//
			// we do not check the original type here and assume
			...
			switch( column.getJavaSqlType() )
			{
			  ...
			  // Strange DB2 Syntax:
			  // A column of type VARCHAR FOR BIT DATA can be extended
			  // with ALTER TABLE...ALTER COLUMN...SET DATA TYPE VARCHAR (without 
			  // 'FOR BIT DATA') in V8.2. Beginning with V9.1
			  // 'FOR BIT DATA' must be added to the SET DATA TYPE clause.
			  ...
			  case java.sql.Types.LONGVARBINARY :
			    ...
			    statement.addLine(   " ALTER COLUMN " + '"' + column.getName().toUpperCase() + '"'
							  	   + " SET DATA TYPE VARCHAR("
								   + ( column.getLength() * column.getJavaSqlTypeInfo().getByteFactor() )
								   + ") " + (dbVersion.compareTo( "SQL09" ) < 0? "" : " FOR BIT DATA ") );
			    break;	
