...
public class DbDb6Index extends DbIndex
{
	...
	private static final int V8_INDEX_NAME_LENGTH = 18;
	...
	public boolean checkNameLength()
	{
		...
		boolean check = (nameLen > 0 && nameLen <= V8_INDEX_NAME_LENGTH);

		loc.entering("checkNameLength");
		if (check == false)
		{
			Object[] arguments = {this.getName()};
			cat.errorT(loc, "checkNameLength for index {0}: index name length must range from 0 to " + V8_INDEX_NAME_LENGTH, arguments);
		}
		loc.exiting();
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.sap.dictionary.database.dbs.DbIndex#checkDbSpecificParameters()
	 */
