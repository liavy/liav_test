...
//    monitoredThread = new MonitoredThreadImpl(shmThread);
  ...
  // call it always after run is already called to guarantee that shm 
  ...
  private void execute(ThreadContextImpl currentThreadCtx) {
    try {
    	if (shmThread != null) {
        shmThread.store();
    	}
      preRun(action);
      AccessController.doPrivileged(action, action.getContext());
    } catch (RuntimeException rex) {
      // Log critical error - uncatched RuntimeException from application thread
  ...
  public void run() {
    try {    
      ...
      if (action != null) {
        try {
          ...
          if (!this.threadManager.isShutDown) {
        	  LoggingUtilities.logAndTrace(Severity.ERROR, catServerCritical, location, td, "ASJ.krn_thd.000029", null, null, 
            				 "Application thread [{0}] got ThreadDeath error while processing [{1}]: [{2}]", 
            				 new Object[]{this.getName(), action, td.toString()});
          } else {
            location.debugT("Application thread [" + this.getName() + "] was stopped when shutting down the engine while processing [" + action + "].");
          }
      ...
      LoggingUtilities.logAndTrace(Severity.ERROR, catServerCritical, location, th,
    		  			"ASJ.krn_thd.000062", null, null,
    		  			"Unrecoverable error thrown while processing tasks: [{0}]", 
    		  			new Object[]{th.toString()});
  ...
  protected void finalize() {  //$JL-FINALIZE$
    if (shmThread != null) {
      shmThread.close();
      shmThread = null;
    }
