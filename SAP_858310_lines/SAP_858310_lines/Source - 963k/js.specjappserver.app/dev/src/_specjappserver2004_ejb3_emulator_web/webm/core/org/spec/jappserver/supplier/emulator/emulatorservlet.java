 ...
 *  2003/05/05  Matt Hogstrom, IBM      Added Delivery Counter to validate Supplier Txs
 */
...
public class EmulatorServlet extends HttpServlet {
    ...
    public  static int deliveryCounter;          // Delivery Counter $Hogstrom 5/5/2003
    ...
    throws ServletException {
        ...
        for( Enumeration _enum = request.getParameterNames();
                ...
                if( "cmd".equals(paramName) ) {
                    if( "switchlog".equals(paramValues[i]) )
                        try {
                            logMgr.switchLog();
                            txCounter = 0;
                            deliveryCounter = 0;               // Reset Delivery Counter $Hogstrom 5/5/2003
                            responseString = "";
                        } catch( IOException e ) {
                        ...
                        try {
                            ...
                            responseString = errorMsg + '\n'
                                             + e.getMessage();
                        } else if( "getEmulatorCount".equals(paramValues[i]) )    // $Hogstrom, added to get Emulator Count
                        try {
                            ...
                            responseString = errorMsg + '\n'
                                             + e.getMessage();
                        } else if( "getDeliveryCount".equals(paramValues[i]) )    // $Hogstrom, added to get Delivery Count
                        try {
                            responseStream.println("TxCount = " + String.valueOf(deliveryCounter) + " ; ");
                            responseString = "";
                        } catch( IOException e ) {
                            responseString = errorMsg + '\n'
                                             + e.getMessage();
        ...
        try {
            if( paramCount == 0 ) {
                ...
                responseStream.println("<html><head><title>EmulatorServlet " +
                                       ...
                                       "<font size=+2>JAS_HOST : " + deliveryConfig.get(
                                                                                       "supplierHost") + "</font><br>" +
                                       "<font size=+2>JAS_PORT : " + deliveryConfig.get(
