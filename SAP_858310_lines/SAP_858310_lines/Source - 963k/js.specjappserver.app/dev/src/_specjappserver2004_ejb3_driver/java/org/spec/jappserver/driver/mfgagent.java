 ...
 *
 * $Id: MfgAgent.java,v 1.5 2004/02/17 17:15:28 skounev Exp $
 */
...
public class MfgAgent extends UnicastRemoteObject 
     ...
     * @param propsFile The property file location
	...
	public Serializable getResults() {
		...
		for (int i = 0; i < numThreads; i++) {
			results[i] = ((PlannedLine)(plannedLineVector.elementAt(i))).getResult();
		}
	...
	public Serializable getCurrentResults() {
		...
		for (int i = 0; i < numThreads; i++) {
			curResults[i] = ((PlannedLine)(plannedLineVector.elementAt(i))).getCurrentResult();
		}
		// Aggregate results from all threads of this agent
    ...
    /**
     * Starts the manufacturing agent and registers it for RMI serving.
	...
	try {
	    ...
	    con.register("MfgAgent", name, mfgAgent);
