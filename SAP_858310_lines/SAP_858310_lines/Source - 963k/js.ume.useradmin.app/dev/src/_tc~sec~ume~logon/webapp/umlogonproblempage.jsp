...
<FORM name="helpForm" method="post" action="<%=inPortal?proxy.getAlias("OLPFS"):logonBean.getLogonURL(proxy, null)%>">
  ...
  <tr>
    ...
    <tr>
      ...
      <td width="183" height="20">
        <input class="urEdfTxtEnbl" id="logonuidfield" name="<%=logonBean.LONGUID%>" type="text" value="<%=EncodeHtmlTag.encode(logonBean.getLongUid(proxy)!=null?logonBean.getLongUid(proxy):"")%>">
      </td>
    </tr>
    <!-- lastname -->
    <tr>
    ...
    <!-- note to admin -->
    <tr>
      <td width="161" height="20" valign="top">
        <label class=urLblStd for="logonnotefield">
          <%=logonLocale.get("NOTE_TO_ADMIN")%>
        </label>
      </td>
      <td width="183" height="20">
        <textarea class="urTedWhl" cols="30" rows="3" id="logonnotefield" name="notetoadmin"></textarea>
      </td>
    </tr>
    <!-- space above buttons -->
    <tr>
    ...
    <tr>
      <td colspan="2">
        <input style="height:3ex;" class="urBtnStd" type="submit" name="OLPFS" value="<%=logonLocale.get("SUBMIT")%>">
