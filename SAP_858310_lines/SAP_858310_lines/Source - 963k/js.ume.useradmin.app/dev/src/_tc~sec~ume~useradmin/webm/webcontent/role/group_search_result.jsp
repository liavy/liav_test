...
<% 
	...
	boolean useUserAsPrincipal = false;
	if ("user".equals( principal ))
	{
   		useUserAsPrincipal = true;
	}
...
function doSearch()
{
	if ((document.frmRoleInfo.cmd.value == null) || (document.frmRoleInfo.cmd.value == ""))
		document.frmRoleInfo.cmd.value = 'group-assignment-search-groups';
}
...
<%@ include file="/contextspecific_includes_top.txt" %>
	...
	<table align="center" cellpadding="0" cellspacing="0" width="99%" border="0">
	    <tr><td width="100%" class="TBLO_XXS_L"><%=roleAdminLocale.get("SEARCH_GROUPS_DESC")%></td></tr>
	</table>
		...
		<tr>
			...
			<% if ( length > 0 ) {%>
				<input class="BTN_LN" type="submit" name="select" value="&nbsp;<%=roleAdminLocale.get("SELECT")%>&nbsp;"
					onClick="setReq('group-assignment-add-groups')">&nbsp;
			<% } %> 
