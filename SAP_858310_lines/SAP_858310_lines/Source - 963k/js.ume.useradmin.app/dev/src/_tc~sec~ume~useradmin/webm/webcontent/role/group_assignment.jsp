...
<% 
	...
	String groupIDs = (String) HelperClass.getAttr(proxy,"groupIDs");
	String groupDescription = (String) HelperClass.getAttr(proxy,"groupDescription");
	...
	String assign_members_group_add = roleAdminLocale.get("ASSIGN_USERS_GROUP_ADD");
	String assigned_members = roleAdminLocale.get("ASSIGNED_USERS_GROUP"); 
	String add_users = roleAdminLocale.get("ADD_USERS");
	String search_members = roleAdminLocale.get("SEARCH_ASSIGNED_USERS");

	if (principal.equals("group"))
	{
    	useUserAsPrincipal = false;
		assign_members = roleAdminLocale.get("ASSIGN_GROUPS");
		assign_members_group_add = roleAdminLocale.get("ASSIGN_GROUPS_GROUP_ADD");
		assigned_members = roleAdminLocale.get("ASSIGNED_GROUPS_GROUP"); 
		add_users = roleAdminLocale.get("ADD_GROUPS"); 
		search_members = roleAdminLocale.get("SEARCH_ASSIGNED_GROUPS");
	}

	if ("MultiGroup".equals(groupDescription))
	{ 
		if (useUserAsPrincipal)
			groupDescription = roleAdminLocale.get("MULTI_USER_GROUP_ASSIGNMENT");
		else
			groupDescription = roleAdminLocale.get("MULTI_GROUP_GROUP_ASSIGNMENT");
	}

	Boolean tempReadOnly = (Boolean)proxy.getRequestAttribute("readOnlyGroups");
...
function doUserSearch(command) 
{
        ...
        inputTag2.setAttribute("value", "<%=groupAdminAlias%>");
...
function doMemberSearch()
{
	if ((document.frmRoleInfo.cmd.value == null) || (document.frmRoleInfo.cmd.value == ""))
		document.frmRoleInfo.cmd.value = 'group-assignment';
}
</script>
...
<%@ include file="/contextspecific_includes_top.txt" %>
	...
	<table cellpadding="2" cellspacing="0" border="0" width="100%" id="h0">
		<tr><td colspan="2"><IMG height="2" src="<%=webpath%>layout/sp.gif" width="1" border="0" alt=""></td></tr>
     	<tr>
			<td class="TBLO_XXS_R" width="25%"><%=roleAdminLocale.get("GROUP_NAME")%></td>
			<td class="TX_XXS"><UM:encode><%=groupnamesReport%></UM:encode></td>
		</tr>
		<tr>
			<td class="TBLO_XXS_R" width="25%" valign="top"><%=roleAdminLocale.get("DESCRIPTION")%>&nbsp;</td>
			<td class="TX_XXS" ><UM:encode><%=groupDescription%></UM:encode></td>
		</tr>
		...
		<tr>
			...
			<% if (!readonly) { %>
				...
				if( useUserAsPrincipal == true) { %>  			  
					onClick="doUserSearch('group-assignment-add-users')">&nbsp;&nbsp;&nbsp;&nbsp;
			    <% } else { %>   		          
					onClick="setCmd('group-assignment-search-groups')">&nbsp;&nbsp;&nbsp;&nbsp;
				<% }
			} %>
		</tr>
	</table>
	...
	<table cellpadding="0" cellspacing="0" border="0" CLASS="TBDATA_BDR_BG" width="98%">
		...
		<% if ( length > 0 ) { %>
			<%@ include file="/role/group_pagenavigation.jsp"%>
		<% } %> 
		<tr>
			<% if (!readonly) { %>
		...
		if (!readonly) { %>
			...
			<% if ( useUserAsPrincipal ) { %>  			  
				onClick="return onRemoveReq('group-assignment-remove-users','<%=util.convertDbQuotationToSingle(roleAdminLocale.get("REMOVE_USER_ALERT"))%>','<%=util.convertDbQuotationToSingle(roleAdminLocale.get("REMOVE_USER_GROUP_ALERT"))%>')">
        	<% } else { %>   		          
				onClick="return onRemoveReq('group-assignment-remove-users','<%=util.convertDbQuotationToSingle(roleAdminLocale.get("REMOVE_GROUP_ALERT"))%>','<%=util.convertDbQuotationToSingle(roleAdminLocale.get("REMOVE_GROUP_GROUP_ALERT"))%>')">
			<% } %>
			</td></tr>
		<% }
	    if ( length > 0 ) { %>
	...
	<table width="98%" border="0" cellpadding="0" cellspacing="0">
		<tr>
			<td width="100%" align="left" class="TBLO_XXS_L" nowrap>
				...
				onClick="setReq('group-abort')">&nbsp;
			</td>
		</tr>
	</table>

<input type="hidden" name="groupnamesReport" value="<UM:encode><%=groupnamesReport%></UM:encode>">
<input type="hidden" name="groupDescription" value="<UM:encode><%=groupDescription%></UM:encode>">
<input type="hidden" name="groupIDs" value="<UM:encode><%=groupIDs%></UM:encode>">
...
<input type="hidden" name="readOnlyGroups" value = "<%=readonly%>">
