...
<table cellpadding="0" cellspacing="1" border="0" class="SEC_TB_BDR" width="98%">
  <tr><td class="TBLO_XXS_L">
    <table id="ga-hd" border="0" cellpadding="0" cellspacing="0">
      <tr class="TBDATA_CNT_EVEN_BG">
            ...
            <img id="ga-exp"
                 ...
                 border="0"
                 alt="<%=altmin%>"
                 onClick="javascript:expandME('ga', 0, '<%=altmin%>', '<%=altmax%>', '<%=webpath%>');"
                 class="IMG_BTN"></td>
	    </tr>
    </table>
  </td></tr>
  <tr><td class="TBDATA_CNT_ODD_BG"><div id="ga-bd">
    <TABLE cellpadding="2" cellspacing="0" border="0" width="100%" id="h2">
	  ...
	  <tr>
        <% if(null!=self)
               ...
               _readOnly = false; %>  	  
		<td class="TBLO_XXS_R" valign="top" nowrap>
          <% if ( _readOnly) { %>
          <% if ( spanTitle.length() > 0 ) spanTitle.delete(0, spanTitle.length());
             spanTitle.append(userAdminLocale.get("NOTE_TO_ADMIN")).append(" ");
             spanTitle.append(user.getNoteToAdmin()).append(" ");
             spanTitle.append(userAdminLocale.get("NOT_AVAILABLE")); %>
          <span title="<%=spanTitle.toString()%>" tabindex="0"><LABEL FOR="<%=user.noteToAdmin%>"><%=userAdminLocale.get("NOTE_TO_ADMIN")%>:</LABEL></span>
          <% } else {  %>
          <LABEL FOR="<%=user.noteToAdmin%>"><%=userAdminLocale.get("NOTE_TO_ADMIN")%>:</LABEL>
          <% } %>		
		</td>
		<td nowrap>
		  <% String noteToAdmin = user.getNoteToAdmin();
		     if ( null == noteToAdmin ) noteToAdmin = ""; %>
		  <textarea id="<%=user.noteToAdmin%>"
		            name="<%=user.noteToAdmin%>"
		            tabindex="0" 
		            value="<%=noteToAdmin%>"
		            cols="20"
		            rows="3"
		            size="20"
		            style="width: 2in"><%=noteToAdmin%></textarea>
        </td>
	  </tr>
      <tr>
        <td colspan="2">
