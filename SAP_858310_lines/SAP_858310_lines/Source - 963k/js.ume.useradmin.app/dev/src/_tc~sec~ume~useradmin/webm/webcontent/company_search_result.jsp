...
<% java.util.Vector temp = list.getObjsOnCurrentPage();
   TradingPartnerInterface[] companies = new TradingPartnerInterface[temp.size()];
   companies = (TradingPartnerInterface[]) temp.toArray(companies);
   ...
   String pageName = CompanySearchLogic.companySearchResultPage;
...
<%}%>
<form name="searchresult" method="post" action="<%=companySearchAlias%>">
...
<table cellpadding="0" cellspacing="0" border="0" width="99%" align="center">
      ...
      <span tabindex="0"><%=userAdminLocale.get("COM_SEARCH_RESULT_HEADER")%></span>
      </td></tr>
</table>
<!-- End Section Header -->
	...
	<table cellpadding="0" cellspacing="0" border="0" CLASS="TBDATA_BDR_BG" width="98%" align="center">
			<tr>
				...
				<input type="radio"
				       tabindex="0"
						name="<%=CompanySearchResultBean.RESULT_COMPANY_ID%>"
						class="noborder"
						value="<%=util.URLEncoder(companies[i].getPartnerID().toString())%>"
						<%=i==0?"checked":""%> ></td>
				<td scope="row" tabindex="0" class="TBDATA_XS_L">
				<UM:encode><%=companies[i].getDisplayName()%></UM:encode>
		...
		<table cellpadding="0" cellspacing="0" border="0">
    <tr><td><a href="#sidemenu"><IMG height="30" src="<%=webpath%>layout/sp.gif" width="1" border="0" alt="skip to sidemenu"></a>
    </td></tr>
    </table>
		...
		<table width="98%" border="0" cellpadding="0" cellspacing="0">
                ...
                <input type="submit"
                       class="BTN_LB"
                       name="<%=CompanySearchLogic.finishCompanySearchAction%>"
                       value=" <%=userAdminLocale.get("SELECT")%> ">&nbsp;
                <input type="button"
                       class="BTN_LN"
                       name="<%=CompanySearchLogic.searchCompaniesAction%>"
                       value=" <%=userAdminLocale.get("SEARCH_AGAIN")%> "
                       onClick="javascript:window.location.href='<%=companySearchAlias%>?<%=UserAdminLogic.selectCompanyAction%>='">
					</td>
				</tr>
			</table></form>
		<!-- End result Table -->
